...
from handlers.common import fmt_date, parse_date, format_board
from keyboards.inline import board_filter_keyboard
from config import ART_DIRECTOR_IDS

router = Router()

# Store filter state per user in FSM
...
# ── /board — art director's view ─────────────────────────────────────────────

@router.message(Command("все_задачи"))
async def cmd_board(message: Message, state: FSMContext):
...
# ── Callbacks: board filters ──────────────────────────────────────────────────

@router.callback_query(F.data.startswith("board_filter:"))
async def cb_board_filter(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(
        text, parse_mode="HTML",
        reply_markup=board_filter_keyboard(
            projects,
...
async def cb_board_refresh(call: CallbackQuery, state: FSMContext):
    if not is_art_director(call.from_user.id):
        await call.answer("Нет доступа.")
        return
    pool = call.bot["pool"]
